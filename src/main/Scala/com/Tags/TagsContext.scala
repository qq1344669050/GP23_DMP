...
object TagsContext {
  def main(args: Array[String]): Unit = {
    ...
    // 处理数据信息
    val res: RDD[(String, List[(String, Int)])] = df.map(row => {
      // 获取用户的唯一ID
      val userId: String = TagUtils.getOneUserId(row)
      // 接下来标签 实现
      val adList = AdTag.makeTags(row)
      // 商圈
      val businessList = BusinessTag.makeTags(row)
      // 媒体标签
      val appList = AppTag.makeTags(row, broadValue)
      // 设备标签
      val devList = DeviceTag.makeTags(row)
      // 地域标签
      val locList = LocationTag.makeTags(row)
      // 关键字标签
      val kwList = KwordTag.makeTags(row, broadValues)
